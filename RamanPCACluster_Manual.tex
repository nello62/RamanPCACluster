...
\code{New session (clear all)} & Reverts the app to its startup state --
no spectra loaded, every preprocessing/clustering control back to its
default -- to begin an unrelated analysis from scratch (confirmation
required). The loaded reference library is deliberately left in place,
since it is a reusable resource rather than a per-analysis result. \\
\end{argtabletwo}
...
\begin{itemize}
  ...
  \item \textbf{Identification} -- one row per $k$-means cluster: its
  size, nearest reference material class and distance, and the runner-up
  class/distance (\S\ref{sec:identification}). Shown only once a
  reference library has been loaded and overlaid.
...
\begin{enumerate}
  ...
  \item \textbf{Identify} (indicative, only if a reference library was
  loaded, \S\ref{sec:identification}): each $k$-means cluster's nearest
  reference material class, by centroid distance in the same retained-PCA
  subspace $k$-means itself clustered on, plus the runner-up class/distance
  to gauge confidence.
...
% -----------------------------------------------------------------------------
\subsection{Cluster identification (nearest-centroid matching)}
\label{sec:identification}
% -----------------------------------------------------------------------------
Once reference spectra sit in the same PCA space as the analyzed samples
(\S\ref{sec:references}), a natural next question is which known material
each $k$-means cluster most resembles. The app answers this with the
simplest classical approach available: \emph{nearest-centroid matching}
(the same principle behind the ``nearest class mean'' / Rocchio
classifier of classical pattern recognition), applied here at the level
of whole clusters rather than individual points.

\paragraph{The matching rule.} Let $\boldsymbol\mu_j$ be cluster $j$'s
centroid -- the mean of its members' scores, in the \emph{same} retained
PCA subspace $k$-means itself clustered on (\S\ref{sec:pca}), not just the
PC1-PC2 plane the scatter plots happen to display -- and
$\boldsymbol\nu_c$ the centroid of reference class $c$'s own spectra in
that identical subspace. The cluster is labeled with the class minimizing
the Euclidean distance,
\[
  \hat c(j) = \arg\min_c \big\lVert \boldsymbol\mu_j - \boldsymbol\nu_c \big\rVert ,
\]
and the runner-up class -- the second-smallest distance -- is reported
alongside it. The gap between the two is the whole point of showing a
runner-up at all: a best match much closer than the runner-up reads as
confident, while two nearly-tied distances reads as genuinely ambiguous
(e.g.\ Polyethylene vs.\ Polypropylene, whose Raman spectra are
notoriously similar) rather than a clean identification.

\begin{tbnote}
\textbf{This is a heuristic, not a classifier.} Unlike LDA
(\S\ref{sec:lda}), nothing here is fitted or cross-validated: there is no
decision boundary learned from held-out data, no estimate of how often
this rule would be right. Two limitations follow directly from that:
(i) $\arg\min$ always returns \emph{some} class, even when the cluster's
true material is not in the reference library at all -- the reported
match is then only the least-wrong guess among what happens to be
available, not evidence that material is actually present (this is the
classic \emph{open-set} problem: a nearest-neighbor/nearest-centroid rule
has no built-in way to say ``none of the above''); (ii) a reference
library assembled from different instruments/samples than the analyzed
spectra can carry its own systematic offset in PCA space unrelated to
material identity. Treat every match as a starting hypothesis to verify
against the raw spectra (e.g.\ known characteristic bands for that
material) -- never as a final identification on its own, particularly
when the runner-up distance is close to the best match's.
\end{tbnote}
...
\begin{itemize}
  ...
  \item \code{cluster\_identification.csv} (only if a reference library
  was loaded) -- one row per $k$-means cluster: its size, nearest
  reference material class and distance, and the runner-up class/distance
  ...
  groups; if used, the reference library's folder, projected scores,
  material classes, names, and the list of references skipped for not
  covering the analysis range; and, if computed, each cluster's best-match
  and runner-up identification classes/distances).
\end{itemize}
